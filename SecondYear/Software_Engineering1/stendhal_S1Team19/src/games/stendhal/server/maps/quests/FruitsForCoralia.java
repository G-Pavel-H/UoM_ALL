...
public class FruitsForCoralia extends AbstractQuest {
    ...
    private void prepareBringingStep() {
		...
		npc.add(ConversationStates.QUESTION_2,
			...
			new QuestActiveCondition(QUEST_SLOT),
			ConversationStates.QUESTION_2,
			"Wonderful, what fresh delights have you brought? If you have brought them all, you can give me #everything!",
			null);
		...
		// Add Trigger for #Everything
		npc.add(ConversationStates.QUESTION_2, "everything", new QuestActiveCondition(QUEST_SLOT),
				ConversationStates.ATTENDING, null,
				new ChatAction() {
					@Override
					public void fire(Player player, Sentence sentence, EventRaiser npc) {
						collectAllItems(player, npc);
					}

					private void collectAllItems(Player player, EventRaiser npc) {
						for (Map.Entry<String, Integer> entry : getMissingItems(player).entrySet()) {
							if (player.drop(entry.getKey(), entry.getValue())) {
								updateQuestStatus(player, entry.getKey(), entry.getValue());
							}
						}
						if (getMissingItems(player).size() > 0) {
							new SayRequiredItemsFromCollectionAction(QUEST_SLOT,
									"You have some missing items. I'd still like [items].").fire(player, null, npc);
						} else {
							completeAction.fire(player, null, npc);
						}
					}

					// Update Recieved Quest Items.
					private void updateQuestStatus(Player player, String itemName, Integer num) {
						final ItemCollection itemsTodo = new ItemCollection();
						itemsTodo.addFromQuestStateString(player.getQuest(QUEST_SLOT));
						itemsTodo.removeItem(itemName, num);
						player.setQuest(QUEST_SLOT, itemsTodo.toStringForQuestState());
					}

					// Get Missing Quest Items.
					private HashMap<String, Integer> getMissingItems(Player player) {
						...
						HashMap<String, Integer> missingItemMap = new HashMap<>();

						for (String item : done) {
							String[] item_quantity = item.split("=");
							if (item_quantity.length == 2)
								missingItemMap.put(item_quantity[0], Integer.parseInt(item_quantity[1]));
						}
