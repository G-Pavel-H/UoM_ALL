...
public class OutfitChangerBehaviour extends MerchantBehaviour {
	 ...
	 */
	@Override
	public boolean transactAgreedDeal(ItemParserResult res, final EventRaiser seller, final Player player, String itemName, String itemType) {
		if(itemType == null) {
			return false;
		}
		outfitType = itemType;
		...
		if (player.isEquipped("money", charge)) {
			...
			putOnOutfit(player, outfitType, itemName);
			return true;
		} else {
	 ...
	 * @param itemName 
	 */
	public void putOnOutfit(final Player player, final String outfitType, String itemName) {
		...
		//Issue here outfit type is same as outfit name
		final List<Outfit> possibleNewOutfits = outfitTypes.get(outfitType);
		
		final Outfit newOutfit;
		
		//switch statement needed for the different masks use possibleNewOutfits.get()
		switch(itemName) {
		case "mouse mask":
			newOutfit = possibleNewOutfits.get(0);
			break;
		case "frog mask":
			newOutfit = possibleNewOutfits.get(1);
			break;
		case "penguin mask":
			newOutfit = possibleNewOutfits.get(2);
			break;
		case "monkey mask":
			newOutfit = possibleNewOutfits.get(3);
			break;
		case "dog mask":
			newOutfit = possibleNewOutfits.get(4);
			break;
		case "cat mask":
			newOutfit = possibleNewOutfits.get(5);
			break;
		default: 
			newOutfit = possibleNewOutfits.get(0);
		}
