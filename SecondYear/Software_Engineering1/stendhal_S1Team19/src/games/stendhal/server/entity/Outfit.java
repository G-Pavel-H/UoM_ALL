...
public class Outfit {
	...
	public Outfit(String strcode) {
		if (strcode.contains("=")) {
			if (strcode.contains("disguise") && !strcode.contains(",disguise")){
				strcode = strcode.replace("disguise", ",disguise");
			}
			final String[] layers;
			if (strcode.contains(",")) {
			...
			LOGGER.error("MAKING OUTFIT" + strcode);
	...
	public Outfit putOver(Outfit old) {
		...
		Integer newDisguise = layers.get("disguise");
		...
		if (newDisguise == null) {
			newDisguise = old.getLayer("disguise");
		}

		return new Outfit(newBody, newDress, newHead, newMouth, newEyes, newMask, newHair, newHat, newDetail, newDisguise);
	}
	...
	public Outfit removeOutfit(final Outfit other) {
		...
		if ((newDisguise == null) || newDisguise.equals(other.getLayer("disguise"))) {
			newDisguise = 0;
		}
	...
	public boolean isPartOf(final Outfit other) {
		...
		Integer dress = layers.get("dress");
		Integer body = layers.get("body");
		Integer disguise = layers.get("disguise");
		...
		return ((hat == null) || hat.equals(other.getLayer("hat")))
				...
				&& ((disguise == null) || disguise.equals(other.getLayer("disguise")))
	...
	public String getData(Map<String, String> colors) {
		...
		sb.append("_disguise-" + getLayer("disguise") + "-");
		sb.append(Integer.toHexString(MathHelper.parseIntDefault(colors.get("disguise"), 0)));
		return sb.toString();
	}
