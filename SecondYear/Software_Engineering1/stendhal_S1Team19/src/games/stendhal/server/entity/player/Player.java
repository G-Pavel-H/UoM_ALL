...
public class Player extends DressedEntity implements UseListener {
	...
	private List<String> disguised_to = new ArrayList<String>();
	...
	public static Player createZeroLevelPlayer(final String characterName,
		...
		if (template != null) {
			if (template.has("outfit_ext")) {
				...
				logger.error("OUTFIT LOGGER" + template.get("outfit_ext"));
	...
	protected void handleSimpleCollision(final int nx, final int ny) {
		...
		} else if (getZone().leavesZone(this, nx, ny)) {
			this.sendPrivateText("You cannot leave this area without your pet nearby.");
		}
		if (isGhost()) {
	...
	public List<String> getDisguisedTo(){
		return this.disguised_to;
	}

	public void setDisguisedTo(List<String> disg){
		this.disguised_to = disg;
	}
