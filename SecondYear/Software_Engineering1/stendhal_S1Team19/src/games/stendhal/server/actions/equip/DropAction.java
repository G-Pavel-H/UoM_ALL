...
public class DropAction extends EquipmentAction {
	...
	protected void execute(final Player player, final RPAction action, final SourceObject source) {
		...
		// check if the zone allows drops first
		if (!player.getZone().checkZone(player, action, entity)) {
			player.sendPrivateText("You cannot drop " + Grammar.plural(source.getEntityName()) + " here.");
			return;
		}
