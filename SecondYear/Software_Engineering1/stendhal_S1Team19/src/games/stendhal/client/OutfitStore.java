...
public class OutfitStore {
	 ...
	 */
	private Sprite buildOutfit(String strcode, final OutfitColor color) {
		...
		Sprite layer;
		logger.error("Outfit code called" + layer_map);
		
		
		if (layer_map.get("disguise") == 0) {
			// Body layer
			final int bodyIndex = layer_map.get("body");
			if (bodyIndex < 0) {
				layer = store.getEmptySprite(48 * 3, 64 * 4);
			} else {
				layer = getLayerSprite("body", bodyIndex, color);
			}

			if (layer == null) {
				throw new IllegalArgumentException(
						"No body image found for outfit: " + bodyIndex);
			}
			...
			logger.error("DISGUISE    " + layer_map.get("disguise"));
			layer = getLayerSprite("disguise", layer_map.get("disguise"));
			sprite = new ImageSprite(layer);
		}
