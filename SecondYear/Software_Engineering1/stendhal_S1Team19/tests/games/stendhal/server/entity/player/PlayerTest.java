...
import games.stendhal.server.entity.item.Disguise;
...
public class PlayerTest {
	 ...
	 */
	@Test
	public void testDisguiseGargoyle(){
		final Player bob = PlayerTestHelper.createPlayer("bob");
		final Disguise disguise = (Disguise) SingletonRepository.getEntityManager().getItem("Disguise Gargoyle Armor");
		assertThat(disguise.describe(), is("You armor that looks like a gargoyle Stats are (DEF: 5 MIN-LEVEL: 0)."));
		bob.equip("armor", disguise);
		assertFalse(bob.getDisguisedTo().contains("gargoyle"));

	}

	@Test
	public void testDisguiseCyclops(){
		final Player bob = PlayerTestHelper.createPlayer("bob");
		final Disguise disguise = (Disguise) SingletonRepository.getEntityManager().getItem("Disguise Cyclops Mask");
		assertThat(disguise.describe(), is("You see a mask that looks like a cyclops Stats are (DEF: 5 MIN-LEVEL: 0)."));
		bob.equip("head", disguise);
		assertFalse(bob.getDisguisedTo().contains("cyclops"));
