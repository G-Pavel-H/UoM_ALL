...
class bnb(knapsack):
    ...
    def frac_bound(self, sol, fix):
        ...
        while (i <= self.Nitems and totalw < self.Capacity):
            ...
            totalw += self.item_weights[self.temp_indexes[i]]
            totalp += self.item_values[self.temp_indexes[i]]
            i += 1
    ...
    def branch_and_bound(self, final_sol):
        ...
        # YOUR CODE GOES HERE
        empty_sol = struc_sol()
        self.frac_bound(empty_sol, 0)
        current_best = empty_sol.val
        self.insert(empty_sol)
        upbound = empty_sol.bound
        fix = 1
        counter = 1
        while self.QueueSize != 0 or upbound < current_best:
            sol = self.removeMax()
            child1 = sol.copy()
            child2 = sol.copy()
            child1.solution_vec[fix] = True
            child2.solution_vec[fix] =  False
            
            self.frac_bound(child1, fix)
            self.frac_bound(child2, fix)
            array = [child1, child2]
            fix += 1
            ...
            for i in array:
                ...
                if i.val == -1:  
                    pass
                if i.val > current_best:
                    current_best = i.val
                    self.copy_array(i.solution_vec, final_sol)
                if i.val != -1:
                    self.insert(i)   
